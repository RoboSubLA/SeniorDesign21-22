...
class BinTask(smach.StateMachine):
    def __init__(self):
        ...
        with self:
            smach.StateMachine.add('position_sub', PositionSub(), transitions={'success': 'drop_ball', 'failed': 'reset_for_reattempt'})
            smach.StateMachine.add('drop_ball', DropBall(), transitions={'success': 'success', 'failed': 'reset_for_reattempt'})
